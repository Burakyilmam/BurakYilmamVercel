async function loadSkills() {
    try {
        const response = await fetch("../jsons/skills.json");

        if (!response.ok) {
            throw new Error(`skills.json yüklenemedi: ${response.status}`);
        }
        ...
        if (!Array.isArray(skills)) {
            console.error("Skills bulunamadı:", language);
            return;
        }
        ...
        console.error("Skills verileri yüklenirken hata oluştu:", error);
    }
}

document.addEventListener("DOMContentLoaded", loadSkills);
document.addEventListener("languageChanged", loadSkills);